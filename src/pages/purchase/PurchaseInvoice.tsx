...
import { collection, query, where, addDoc, updateDoc, deleteDoc, doc, serverTimestamp, onSnapshot, orderBy, runTransaction } from 'firebase/firestore';
import { db } from '../../lib/firebase';
import { useAuth } from '../../hooks/useAuth';
import { PurchaseInvoice, PurchaseOrder, Supplier, BankAccount } from '../../types';
import { Plus, Search, Edit2, Trash2, FileText, X, Printer, DollarSign, Calendar, Clock, Landmark, ArrowRight, CheckCircle2 } from 'lucide-react';
...
export default function PurchaseInvoices() {
  ...
  const handleDelete = async (id: string) => {
    setConfirmConfig({
      isOpen: true,
      title: 'Hapus Invoice',
      message: 'Apakah Anda yakin ingin menghapus tagihan supplier ini?',
      type: 'danger',
      onConfirm: async () => {
  ...
  const handlePrint = (pi: PurchaseInvoice) => {
        ...
        <body>
          ...
          <div class="info-grid">
            ...
            <div style="text-align: right;">
              ...
              <div style="font-weight: bold; font-size: 16px;">${profile?.tenantName || 'Our Company'}</div>
              <div>PO Ref: ${pi.poNumber || '-'}</div>
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <div className="flex space-x-1">
                <button onClick={() => handlePrint(pi)} className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                  <Printer className="w-4 h-4" />
                </button>
                <button onClick={() => { setEditingInvoice(pi); setFormData({ poId: pi.poId, amount: pi.amount, dueDate: pi.dueDate, status: pi.status }); setIsModalOpen(true); }} className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(pi.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <div>
              ...
              <h3 className="font-bold text-gray-900">{pi.piNumber}</h3>
              <p className="text-xs text-gray-500">Ref PO: {pi.poNumber}</p>
            </div>

            <div className="flex justify-between items-end pt-2">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase">Total Tagihan</p>
                <p className="text-lg font-black text-red-600">Rp.{pi.amount.toLocaleString()}</p>
              </div>
            ...
            <div className="pt-4 border-t border-gray-50 flex items-center justify-between text-xs text-gray-500">
              <div className="flex items-center">
                ...
                Jatuh Tempo: {new Date(pi.dueDate).toLocaleDateString()}
              </div>
              {new Date(pi.dueDate) < new Date() && pi.status !== 'paid' && (
            ...
            {pi.status !== 'paid' && (
              <button
                ...
                className="w-full mt-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
              >
