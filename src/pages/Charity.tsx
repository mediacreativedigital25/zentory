...
export default function Charity() {
  ...
  const handleSaveCharity = async () => {
    ...
    setIsSaving(true);
    try {
      const totalProfit = selectedClosing.grossProfit;
      ...
      await addDoc(collection(db, 'transactions'), {
        ...
        description: `Amal tgl ${dateStr} (Ref: ${selectedClosing.dailyNumber})`,
  ...
  const handleRequestRevision = async () => {
    ...
    try {
      await addDoc(collection(db, 'approval_requests'), {
        ...
        closingId: selectedCharityForRequest.dailyClosingId,
                        ...
                        <td className="px-8 py-6 text-right">
                          {hasCharity ? (
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-indigo-600 text-white text-xs font-black rounded-xl hover:bg-indigo-700 transition-all flex items-center ml-auto"
                            >
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-6">
              {charityRecords.map((record) => (
                <div key={record.id} className="p-6 rounded-3xl bg-gray-50 border border-gray-100 relative group">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="text-[10px] font-black text-gray-400 uppercase">{record.dailyNumber}</p>
                      <p className="text-xs font-bold text-gray-600">
                        {new Date(record.date.seconds * 1000).toLocaleDateString('id-ID')}
                      </p>
                    </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-8">
                ...
                <div className="p-8 bg-indigo-50 rounded-[2.5rem] border border-indigo-100 space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-indigo-900">Total Profit (Gross):</span>
                    <span className="text-xl font-black text-indigo-600">Rp.{selectedClosing.grossProfit.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-pink-700">Amal (2.5%):</span>
                    <span className="text-xl font-black text-pink-600">- Rp.{(selectedClosing.grossProfit * 0.025).toLocaleString()}</span>
                  </div>
                  <div className="pt-4 border-t border-indigo-200 flex justify-between items-center">
                    <span className="text-lg font-black text-indigo-900">Profit Bersih:</span>
                    <span className="text-2xl font-black text-indigo-600">
                      Rp.{(selectedClosing.grossProfit - (selectedClosing.grossProfit * 0.025)).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-6 py-4 bg-red-600 text-white rounded-2xl font-black hover:bg-red-700 disabled:opacity-50 transition-all flex items-center justify-center"
                  >
