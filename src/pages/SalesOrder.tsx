...
import { ShoppingCart, Search, Plus, Minus, Trash2, CreditCard, User, Tag, Briefcase, X, ListOrdered, Barcode, Landmark } from 'lucide-react';
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-120px)]">
          ...
          <div className="flex gap-4">
            <form onSubmit={handleBarcodeScan} className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-indigo-50 border border-indigo-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-indigo-900 placeholder:text-indigo-300"
              />
            </form>
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          </div>
          <div className="flex gap-4">
      ...
      <div className="w-full lg:w-96 flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Payment Method / Bank</label>
            <select 
              value={selectedBankAccountId}
              onChange={(e) => setSelectedBankAccountId(e.target.value)}
              className="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500"
            >
              {bankAccounts.length === 0 && <option value="">No Bank Accounts Set</option>}
              {bankAccounts.map(b => (
